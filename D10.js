...
console.log(splitMe("Questo non è un valore numerico"))
...
console.log(onlyLetters("Qu3st0 corso è d1vertene"))
...
function isTodayMyBirthday() {
  const today = new Date();
  const myBirthDay = new Date(today.getFullYear(), 4, 16);
...
console.log(deleteProp(cars, "modello"))
...
function onlyInLastMillennium(movies) {
  return movies.filter(movies => movies.Year >= 1020 && movies.year <= 2020)
}
...
const searchResult = searchByTitle("Avengers")
function searchByTitle(searchString) {
  return movies.filter(movies => movies.Title.toLowerCase().includes(searchString.toLowerCase()))
}
...
const updateMovies = removeIndex(7)
